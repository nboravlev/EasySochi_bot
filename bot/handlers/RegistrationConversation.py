...
"В глобальном смысле, это настраиваемый масштабируемый продукт для управления бизнесом в сфере услуг, аренды, проката и т.п. По вопросам разработки и внедрения также можно написать мне в бота"
...
async def handle_problem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if problem_text.lower() in ("/help", "help"):
            await update.message.reply_text(
                "⚠️ Опишите проблему, и я передам сообщение администратору."
            )
